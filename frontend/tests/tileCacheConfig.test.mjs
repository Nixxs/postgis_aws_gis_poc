...
const baseConfig = {
  ...
  layers: [{
    ...
    opacity: 0.4,
    color: '#1976d2',
    cache: { enabled: true, grid: 'webmercator', minZoom: 0, maxZoom: 12, fields: '*' },
  }, {
    id: 'uncached',
    label: 'Uncached',
...
test('cache-enabled layers resolve immutable CloudFront URLs once', async () => {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    if (String(url).endsWith('/latest.json')) return jsonResponse({
      version: 'release-1',
      baseKey: 'tiles/public/planning/webmercator/release-1',
      manifestKey: 'tiles/public/planning/webmercator/release-1/tilejson.json',
    })
  ...
  try {
    ...
    assert.equal(first.layers[0].resolvedCache.tileUrl, 'https://tiles.example.test/tiles/public/planning/webmercator/release-1/{z}/{x}/{y}.mvt')
    assert.equal(first.layers[0].resolvedCache.maxZoom, 12)
...
test('cache discovery failure falls back to the dynamic API when configured', async () => {
  const oldFetch = globalThis.fetch
