...
async function resolveLayerCache(config: AppConfig, layer: LayerConfig): Promise<LayerConfig> {
  ...
  const grid = layer.cache.grid ?? 'webmercator'
  if (grid !== 'webmercator') throw new Error(`MapLibre layer '${layer.id}' requires a Web Mercator cache`)
  ...
  const configuredMaxZoom = layer.cache.maxZoom ?? 12
